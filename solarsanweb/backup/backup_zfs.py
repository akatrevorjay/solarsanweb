...
class Dataset:
    ...
    def get_snapshots(self, flt=True):
        if flt is True:
            flt = lambda x: True
        children = [c for c in self.children if isinstance(c, Snapshot)
                    and flt(c)]
        return children
    ...
    def lookup(self, name):  # FINISH THIS
        ...
        if '/' not in path:
            try:
                dset = self.get_child(path)
            except KeyError:
                raise KeyError, 'No such dataset %s at %s' % (path,
                        self.get_path())
            if snapshot:
                try:
                    dset = dset.get_snapshot(snapshot)
                except KeyError:
                    raise KeyError, 'No such snapshot %s at %s' \
                        % (snapshot, dset.get_path())
        else:
            (head, tail) = path.split('/', 1)
            try:
                child = self.get_child(head)
            except KeyError:
                raise KeyError, 'No such dataset %s at %s' % (head,
                        self.get_path())
            if snapshot:
                tail = tail + '@' + snapshot
...
class PoolSet:  # maybe rewrite this as a dataset or something?
    ...
    def lookup(self, name):
        ...
        if '/' not in path:
            try:
                dset = self.pools[path]
            except KeyError:
                raise KeyError, 'No such pool %s' % name
            if snapshot:
                try:
                    dset = dset.get_snapshot(snapshot)
                except KeyError:
            ...
            (head, tail) = path.split('/', 1)
            try:
                pool = self.pools[head]
            except KeyError:
                raise KeyError, 'No such pool %s' % head
            if snapshot:
                tail = tail + '@' + snapshot
    ...
    def parse_zfs_r_output(self, output, creationtimes):
        ...
        creations = dict([s.strip().split("	") for s in
                         creationtimes.splitlines() if s.strip()])
        ...
        for dset in new_dsets:
            ...
            (poolname, pathcomponents) = (dset.split('/')[0],
                    dset.split('/')[1:])
